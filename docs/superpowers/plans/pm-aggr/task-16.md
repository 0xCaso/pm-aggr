...
**Files:**
- Create: `src/components/QuotePanel.tsx`

**Dependencies:** Task 12 (useOrderBook hook provides the data interface)

**Context:** Dollar amount input + YES/NO toggle + debounced quote result. When user types an amount and selects a side, it calls `POST /api/quote` after 300ms debounce. Displays total shares, weighted avg price, per-venue breakdown, and book exhausted warning.

---

- [ ] **Step 1: Create `src/components/QuotePanel.tsx`**
...
```bash
git add src/components/QuotePanel.tsx
git commit -m "feat: add QuotePanel with debounced quote fetch and venue breakdown"
```
