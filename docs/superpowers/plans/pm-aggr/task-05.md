...
- Create: `tests/lib/quote.test.ts`

**Dependencies:** Task 2 (types)

**Context:** The quote engine walks the asks (cheapest to most expensive) for the requested side, greedily filling until the dollar budget is spent. It returns total shares, average price, per-venue breakdown, and whether the book was exhausted.

---

- [ ] **Step 1: Write the quote tests**
...
Run: `npx vitest run tests/lib/quote.test.ts`

Expected: FAIL — `Cannot find module '@/lib/quote'`

- [ ] **Step 3: Implement `calculateQuote`**
...
```bash
git add src/lib/quote.ts tests/lib/quote.test.ts
git commit -m "feat: add quote engine with greedy walk algorithm and tests"
```
