...
**Context:** Displays connection status for the relay and each venue. Green dot = connected, yellow dot = reconnecting, red dot = disconnected. Shows stale data warning if no update in 30s. Collapses to a thin bar when all connected and not stale.
...
git commit -m "feat: add StatusBanner component with connection dots and stale warning"
```
