...
**Dependencies:** Tasks 12-16 (hook + all components)

**Context:** Wire everything together on the main page. The page uses `useOrderBook` to get live data, then passes it to `StatusBanner`, `MarketHeader`, `OrderBook`, and `QuotePanel`. The page also manages the YES/NO side state that's shared between the order book view and the quote panel.
...
Expected: Build completes with no errors. Output shows `/` route compiled.

- [ ] **Step 3: Commit**

```bash
git add src/app/page.tsx
git commit -m "feat: wire up main page with order book, quote panel, status banner"
```
