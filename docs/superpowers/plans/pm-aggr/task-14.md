...
**Files:**
- Create: `src/components/MarketHeader.tsx`

**Dependencies:** Task 12 (useOrderBook hook provides the data interface)

**Context:** Displays the market title and per-venue mid prices. The mid price is `(bestBid + bestAsk) / 2` for each venue's YES book. Prices are formatted as cents (e.g., "37.25¢"). Filters bids/asks by venue to compute per-venue mids.

---

- [ ] **Step 1: Create `src/components/MarketHeader.tsx`**
...
```bash
git add src/components/MarketHeader.tsx
git commit -m "feat: add MarketHeader with market title and per-venue mid prices"
```
