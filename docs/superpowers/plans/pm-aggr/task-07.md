...
**Files:**
- Create: `src/server/connectors/polymarket.ts`

**Dependencies:** Tasks 2 (types), 3 (normalization)

**Context:** The Polymarket connector connects to the CLOB WebSocket, subscribes to the YES token, handles full `book` snapshots and incremental `price_change` updates. It maintains an internal `Map<string, { price: number; size: number }>` per side for efficient level updates. On `price_change`, size `"0"` means remove that level. On reconnect, fetch a full snapshot via REST.

---

- [ ] **Step 1: Create `src/server/connectors/polymarket.ts`**
...
```bash
git add src/server/connectors/polymarket.ts
git commit -m "feat: add Polymarket CLOB connector with incremental updates"
```
